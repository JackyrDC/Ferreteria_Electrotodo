...
export default function SidebarLayout({ activeOption, onSelect }: SidebarLayoutProps) {
  // Función para manejar clicks
  const handleClick = (option: SidebarOption) => {
    onSelect(option);
  };

  // Función para calcular clases según activo o no
  const getLinkClasses = (option: SidebarOption) =>
    `flex items-center p-2 rounded-lg ${
      activeOption === option ? 'bg-[#D71B07] text-white' : 'text-[#A1A1A1] hover:bg-[#D71B07] hover:text-white'
    } group`;
        ...
        <div className="px-3 py-3 lg:px-5 lg:pl-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center justify-start rtl:justify-end">
              <button
                type="button"
                className="inline-flex items-center p-2 text-sm text-[#A1A1A1] rounded-lg sm:hidden hover:bg-[#FFB86A] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#FFB86A]"
              >
              ...
              <a href="#" className="flex ms-2 md:me-24">
                <img
                  src="https://i.pinimg.com/736x/0d/ca/53/0dca53215350af8eae0ae904018c852c.jpg"
                  className="h-8 me-3"
                  alt="Logo"
                />
                <span className="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap text-[#D71B07]">
                  Ferreteria Electrotodo
                </span>
              </a>
            </div>
          </div>
          ...
          <ul className="space-y-2 font-medium">
            <li>
              <button
                ...
                className={getLinkClasses('empleados')}
                type="button"
              >
            ...
            <li>
              <button
                onClick={() => handleClick('ordenCompra')}
                className={getLinkClasses('ordenCompra')}
                type="button"
              >
                <svg className="w-5 h-5 transition duration-75 group-hover:text-white" fill="currentColor" viewBox="0 0 20 20">
            ...
            <li>
              <button
                onClick={() => handleClick('recepcionCompras')}
                className={getLinkClasses('recepcionCompras')}
                type="button"
              >
                <svg className="w-5 h-5 transition duration-75 group-hover:text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <span className="ms-3">Recepción de Compras</span>
              </button>
            </li>
            <li>
              <button
            ...
            <li>
              <button
                ...
                className={getLinkClasses('signOut')}
                type="button"
              >
                <svg className="w-5 h-5 transition duration-75 group-hover:text-white" fill="currentColor" viewBox="0 0 20 20">
